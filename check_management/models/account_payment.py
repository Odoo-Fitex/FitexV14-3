...
class AccountPayment(models.Model):
    ...
    total_check_amount = fields.Float(string="Total Check Amount", compute="compute_total_check_amount", store=True, default=0.0)
    ...
    show_rate = fields.Boolean(string="Show Rate", compute='change_rate_for_the_check_lines')
    journal_currency = fields.Many2one('res.currency', 
                                        string='Jounal Currency',
    ...
    # make cancel function but not delete and undpost the journal entry
    # @api.multi
    def cancel2(self):
        for rec in self:
