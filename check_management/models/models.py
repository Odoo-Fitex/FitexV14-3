...
class PaymentCheckLine(models.Model):
    ...
    rate = fields.Float(string="Rate", default=1, required=True) 
    show_rate_related = fields.Boolean(string="Show Rate", related='payment_id.show_rate')
#      company_currency = fields.Many2one('res.currency', string='Currency', compute='change_rate_for_the_check_lines')
...
#             rate = self.env['res.currency.rate'].search([('currency_id','=',self.currency_id.id),('name','=',self.check_date)])
#             rate_value = rate.rate
            check_amount2 = self.check_amount*self.rate
            move_line_1 = {
                # 'account_id': self.payment_id.partner_id.property_account_receivable_id.id,
                'account_id': self.payment_id.journal_id.default_debit_account_id.id,
                'debit': check_amount2,
                ...
                'partner_id': self.payment_id.partner_id.id,
                'journal_id': self.payment_id.journal_id.id,
                'amount_currency':self.check_amount,
            ...
            move_line_2 = {
                ...
                'account_id': self.payment_id.partner_id.property_account_payable_id.id,
                'debit': 0.0,
                'credit': check_amount2,
                'partner_id': self.payment_id.partner_id.id,
                'journal_id': self.payment_id.journal_id.id,
                'amount_currency':self.check_amount * -1,
                'currency_id':self.currency_id.id,
            }
            lines = [(0, 0, move_line_1), (0, 0, move_line_2)]
            move_vals = {
...
#     @api.one
    ...
    def write(self, vals):
        ...
        if 'state' in vals:
            for line in self:
                self.env['check.history'].create({'check_id': line.id,
...
#     def change_rate_for_the_check_lines(self):
#         self.company_currency = self.env.company.currency_id.id
#         if  self.company_currency.id == self.currency_id.id:
